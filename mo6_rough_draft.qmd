...
# Hydrate the final model into a full copy of the GTD dataset, mapping over via eventid
completed_data <- complete(final_imputation, action = "long")
imputed_subset <- completed_data %>%
  select(eventid, nperps, nperps_cat, nkill, nwound)

gtd_clean_final <- gtd_clean %>%
  select(-c(nperps, nperps_cat, nkill, nwound)) %>%
  left_join(imputed_subset, by = "eventid")

# Verify the missingness is gone
summary(gtd_clean_final$nperps)
```
\clearpage
