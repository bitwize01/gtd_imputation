...
zinb_models <- with(final_imputation, 
...
```{r rootogram_plot, include=FALSE}
# ZINB model
plot_zinb <- zeroinfl(nkill ~ nperps_cat + region | attacktype1 + weaptype1, 
                      data = gtd_clean_final, dist = "negbin")

# Standard Negative Binomial for comparison
plot_nb <- glm.nb(nkill ~ nperps_cat + region + attacktype1 + weaptype1, 
                  data = gtd_clean_final)
```
``` {r rootogram_plot_2, include=FALSE}
# Had to go rip the source code for this out of the rootogram github after some
# conflicts between this package and others


```
``` {r rootogram_plot_3, include=FALSE}
plot_limit <- 25 
max_count <- max(gtd_clean_final$nkill, na.rm = TRUE)

# Extract the mean (mu) and dispersion parameter (theta)
mu_nb <- predict(plot_nb, type = "response")
theta_nb <- plot_nb$theta

# Calculate expected frequencies: sum the probabilities of exactly 'i' deaths across all attacks
...
# The pscl package natively calculates the full probability matrix using type = "prob"
...
df_nb <- data.frame(
  ...
  mutate(
    Sqrt_Obs = sqrt(Observed),
    Sqrt_Exp = sqrt(Expected),
    Bottom = Sqrt_Exp - Sqrt_Obs # The bar hangs down from the expected line
  )
df_zinb <- data.frame(
  Count = 0:plot_limit,
  Observed = as.numeric(observed_freq),
...
# Helper function to generate the rootogram plot to avoid repeating code
...
#| label: fig6
#| fig-cap: "Negative Binomial Regressions: nkill"
# Create both plots
plot1 <- build_rootogram(df_nb, "Standard Negative Binomial")
plot2 <- build_rootogram(df_zinb, "Zero-Inflated Negative Binomial")
final_rootogram <- grid.arrange(plot1, plot2, ncol = 2)
# Display in viewer
print(final_rootogram)
# Save for publication
ggsave("rootogram_comparison.png", plot = final_rootogram, width = 12, height = 6, dpi = 300)
```
