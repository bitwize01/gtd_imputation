...
[@wani2024review] explores the mechanisms of missing data completely at random (MCAR), when the observed data is unevenly missing at random (MAR) and not missing at random (NMAR), in which the missingness is a result of a confounding effect. In their review of potential imputation frameworks, Wani evaluates recovery strategies such as Multiple Imputation by Chained Equations (MICE), Random Forests, and Generative Adversarial Networks. Wani advocates for task-specific evaluation metrics rather than relying solely on standard error calculations to address imbalanced NMAR datasets such as the GTD which exhibit high dimensionality. In datasets such as these, fields are left blank or intentionally flagged as unknown if there is not clear concensus, even if some potential information points toward a value. Indeed, Abdasalam notes in their 2024 article exploring the missingness of the group name field that a value being left unknown is sometimes a concious choice, stating "This gap in research may stem from the complex and dynamic nature of terrorist groups, which frequently evolve, change names, merge, or disband, thereby complicating the creation of accurate and current classification and prediction models." [@Abdalsalam2024a].

Reductions in the analyzed dataset have also prevailed to produce statistically significant predictons at the cost of external validity. Abdalsalam has had success with highly predictive contextual prediction models such as DistilBERT [@Abdalsalam2021a] or BiGRU [@Abdalsalam2024a], but only with a small collection of potential groups who have a 500 or more documented attacks in the GTD. This exposes the limitations of predicting a 'freeform' field such as group name using a statistical model with a limited Retrieval Augmented Generation (RAG) ability to narrow the search space of the contextual model. In the tangential field of Biomedical data, Azad has shown the promise of ensemble-based methods that combine contextual models with ensemble models to produced balanced approaches that adjust weights of predictors across multiple missing fields [@Azad2025]. This approach may allow analysts to bridge the gap between records where generating a RAG of applicable context (such as English language news articles) is not available in a timely banner.

Accounting for the level of missingness using a mask indicator is also useful to identify patterns in NMAR datasets. Using this method, Perez-Lebel (2022) benchmarked imputation approaches on healthcare databases and found that tree-based models utilizing Missing Incorporated in Attributes (MIA) provide the best balance of accuracy and speed [@PerezLebel2022]. When data is weakly correlated, a Ranked k-Nearest Neighbors (RkNN) can perform well. @Khan2024 introduces the RkNN algorithm to refine traditional kNN imputation. By utilizing Ranked Set Sampling, this algorithm better handles data variability and reduces error when variables are weakly correlated or lack sufficient nearby neighbors.
