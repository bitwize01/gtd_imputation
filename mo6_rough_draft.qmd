...
After a careful effort of data imputation and synthesis, my analysis has indicates a statistically significant difference in casualty outcomes for individual vs. group attacks, although differences in tactics vary only slightly between the two groups.
...
ggplot(acled_missingness_summary, aes(x = Missing_Pct, y = reorder(acled_target_col_labels, Column))) +
  geom_col(fill = "steelblue", alpha = 0.8) +
  geom_text(aes(label = sprintf("%.1f%%", Missing_Pct)), 
            vjust = -0.5, size = 3.5) +
  scale_x_continuous(limits = c(0, 100), expand = expansion(mult = c(0, 0.1))) +
  scale_y_discrete(labels = acled_target_col_labels) +
  labs(
...
When examining the effect that tactics and group composition has on casualties in terror attacks, I was concerned that the extreme overdispersion of casualties would result in a standard regression model underestimating the standard errors, leading to false significance. This is a particular risk when considering the imputed dataset, which may already have been overrepresenting the number of individual perpetrators. As a result, I implemented a Zero-inflated negative binomial (ZINB) model to examine the relationship between nkill and the other predictors. The ZINB model separates the structurally missing (data not determined by GTD) data from the attack data where the attack was a failure. However, upon production of the model, its predictors bias matched the Negative Binomial result. The models slightly under-predicts single fatalities at the expense of an over-prediction of all other values, but overall the model performs well.
